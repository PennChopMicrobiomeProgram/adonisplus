example_data <- tibble::tibble(
  SampleID = paste0(c(rep("C", 5), rep("P", 6)), c(1:5, 1:6)),
...
example_dist <- structure(
  ...
  Size = 11L,
  call = quote(as.dist.default(m = cp_dist)),
  class = "dist",
  Diag = FALSE,
  Upper = FALSE
)
...
test_that("adonisplus works for two groups", {
  observed <- adonisplus(
    example_data,
    example_dist,
    distmat ~ study_group * time_point,
    rep_meas_var = SubjectID,
...
test_that("adonisplus works for two groups with covariate", {
  observed <- adonisplus(
    example_data,
    example_dist,
    distmat ~ age + study_group * time_point,
    rep_meas_var = SubjectID,
    shuffle = c(study_group = "between", time_point = "within"),
    permutations = 9
  )
