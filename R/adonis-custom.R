...
adonisplus <- function(data, distmat, formula, sample_id_var = SampleID,
                       ...
                       permutations = 999, seed = 42, adonis2_by="terms") {
  sample_ids <- data %>%
  ...
  a_observed <- vegan::adonis2(
    formula = formula, data = data, permutations = permutations, by=adonis2_by
  )
  ...
  if (!is.null(shuffle)) {
    ...
    fs_permuted <- replicate(permutations, {
      ...
      trial_a <- vegan::adonis2(formula, trial_data, permutations = 4, by=adonis2_by)
