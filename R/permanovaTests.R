...
#' @export
permanova_with_shuffle_2_groups <- function(dist_matrix, s_toTest,
                                            SampleID,
                                            group_label1, group_label2,
                                            rep_mes_label,
                                            covariates, perm,
                                            first_within=F, second_within=F) {
  group_label1 <- rlang::enquo(group_label1)
  group_label2 <- rlang::enquo(group_label2)
  rep_mes_label <- rlang::enquo(rep_mes_label)
  SampleID <- rlang::enquo(SampleID)

  set.seed(42)
  s_toTest <- as.data.frame(s_toTest)
  dist_toTest <- usedist::dist_subset(dist_matrix, as.character(dplyr::pull(s_toTest, !!SampleID)))
  form1 <- paste("dist_toTest", "~", rlang::quo_text(group_label1), " * ", rlang::quo_text(group_label2))

  if (!is.na(covariates)) {
    form1 <- paste(form1, " + ", covariates)
  }
  a_ixn_orj <- vegan::adonis(as.formula(form1), data=s_toTest, permutations=perm)
  ...
  fs_permuted <- replicate(perm, {
    ...
    if (first_within) {
      s_permuted <- s_permuted %>%
        dplyr::mutate(!!group_label1 := shuffle_within_groups(!!group_label1, !!rep_mes_label))
    } else {
      s_permuted <- s_permuted %>%
        dplyr::mutate(!!group_label1 := shuffle_between_groups(!!group_label1, !!rep_mes_label))
    }

    if (second_within) {
      s_permuted <- s_permuted %>%
        dplyr::mutate(!!group_label2 := shuffle_within_groups(!!group_label2, !!rep_mes_label))
    } else {
      s_permuted <- s_permuted %>%
        dplyr::mutate(!!group_label2 := shuffle_between_groups(!!group_label2, !!rep_mes_label))    }
